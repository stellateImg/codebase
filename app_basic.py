...
def pcaanalysis():  
    if request.method == 'POST':  
        ...
        response=find_PCAKmeans(os.path.join(app.config["UPLOAD_PCA"], image1_path.filename),os.path.join(app.config["UPLOAD_PCA"], image2_path.filename))
        print(response)
...
def find_PCAKmeans(imagepath1, imagepath2):
    ...
    response =  { 'status' : 'Success', 'imagePath': 'ChangeMap.jpg' }
    return response
...
def success():  
    if request.method == 'POST':
        ...
        #  files = [(zipfile_ob.open(name).read(),name) for name in file_names]
         ...
         print("-------------------dffdnbfdf===========================================")
         print(files)
         files.save(os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))


         tp1 = getImage(satellite, 1, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
         ...
         if type(tp1) == type(None):
            ...
            im = Image.open(os.path.join('static/playground_output/', unique_filename+'.tif'))
            im.save(outfile, "JPEG", quality=100)
            
            image_list.append('static/playground_output/'+unique_filename+'.jpg')
         
         tp2 = getImage(satellite, 2, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
         ...
         tp3 = getImage(satellite, 3, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
         ...
         tp4 = getImage(satellite, 4, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
         ...
         tp5 = getImage(satellite, 5, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
         ...
         tp6 = getImage(satellite, 6, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
         ...
         tp7 = getImage(satellite, 7, os.path.join(app.config["PLAYGROUND_INPUT"], files.filename))
...
def segmentation():  
    if request.method == 'POST':
        ...
        tarr = cv2.resize(convert_into_rgb(filepath,os.path.join('static/segmentation_upload/', unique_filenameinput+'.jpg')), shape_to_resize)
        ...
        plt.imsave(os.path.join('static/segmentation_upload/', unique_filename+'.png'),np.squeeze(myoutput), cmap="gray")
        

        print("\nSAVED SUCCESFULLY!!\n")
        response =  { 'Status' : 'Success', 'input': os.path.join('static/segmentation_upload/', unique_filenameinput+'.jpg'), 'output':os.path.join('static/segmentation_upload/', unique_filename+'.png')}
        print(response)
